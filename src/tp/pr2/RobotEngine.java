...
//	private CodeCard codeCard;
	...
	public void startEngine(){
		...
		System.out.println(this.initialPlace.toString() + LINE_SEPARATOR + 
				"   * My power is " + power+ LINE_SEPARATOR + "   * My recycled material is: " + reciclado + 
				LINE_SEPARATOR +"WALL·E is looking at direction " + this.direction.toString()); // Primer punto
	...
	public void instruccionTurn (){
		...
		if ( this.instruction.getRotation() == Rotation.LEFT) {
			...
			if ( this.direction == Direction.EAST){
				...
				System.out.println( "WALL·E says: I do not understand. Please repeat" + (LINE_SEPARATOR));
	...
	public void instruccionMove (){
		...
		if(i==1){
			this.power -=5;
			System.out.println("WALL·E > WALL·E >  WALL·E says: Moving in direction"+ this.direccion()); 
			System.out.println(initialPlace.toString());
			System.out.println("   *My power is " + power + LINE_SEPARATOR + "   *My recycled material is: " + reciclado
					+ LINE_SEPARATOR +"WALL·E is looking at direction " +  this.direction); // Primer punto
		}
		else{
			System.out.println("WALL·E > WALL·E says: Arrggg, there is a street but it is closed!");
		}
	}	
		...
		if ( this.container.existsItem(id) ){
			...
			if (it.canBeUsed()){
				if(!it.use(this, this.initialPlace)){
					System.out.println( "WALL·E > WALL·E says: I have problems using the object " + id );
				}
	...
	public void instruccionPick (){
		...
		if( this.initialPlace.existItem(aux)){
			...
			if( this.container.addItem(item)){
				System.out.println("WALL·E > WALL·E says: I am happy! Now I have  " + item.id );
				
			}
		} 
		else 
		{
		System.out.println("WALL·E > WALL·E says: Ooops, this place has not the object " + aux);
		}
	}
	...
	public void instruccionScan (){
		...
		if(id==""){
			if (container.numberOfItems() == 0){
				System.out.println ("WALL·E > WALL·E says: My inventory is empty");
			} else {
				System.out.println ("WALL·E > WALL·E says: I am carrying the following items");
			...
			if (container.existsItem(id)){
				System.out.print("WALL·E > WALL·E says: ");
				System.out.println(it.toString());
			}
			else{
				System.out.println( "WALL·E > WALL·E says: I have not such object");
			}	
		}
	}
